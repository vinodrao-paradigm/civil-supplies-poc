...
col4.metric("DBT Fraud Risk Score", fraud_risk, f"{dbt_anomalies} alerts/10k")
col5.metric("Estimated Annual Savings (₹ Cr)", fiscal_savings, "Simulated")
# ---------- TAB 6: SIMPLE DEMO CHATBOT (FAKE AI) ----------
with tab6:
    st.subheader("AI Assistant (Demo)")

    st.markdown("""
    Ask questions about the Civil Supplies AI Command Centre.
    This is a **simulated** chatbot to demonstrate how an AI assistant will work.
    """)
    ...
    if user_input:
        ...
        if "leakage" in query:
            answer = "Leakage is detected by checking truck GPS deviations and mismatches in stock movement."
        elif "ghost" in query:
            answer = "Ghost beneficiaries are flagged using Aadhaar deduplication, inactivity checks, and pattern matching."
        elif "dbt" in query:
            answer = "DBT fraud is detected when the system sees unusual withdrawal bursts or mismatched identities."
        elif "quality" in query:
            answer = "Grain quality is inspected using AI-based image analysis and warehouse IoT sensors."
        else:
            answer = "This is a demo chatbot with simple canned answers. The real version will use AI to answer intelligently."
